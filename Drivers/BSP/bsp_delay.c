...
void SysTick_Handler(void)
{
   /* When the scheduler is running call the FreeRTOS tick handler so the
    * kernel can manage time and unblock/delay tasks. If the scheduler has not
    * yet been started, don't call into the kernel. */
   if(xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED)
   {
       xPortSysTickHandler();
   }
