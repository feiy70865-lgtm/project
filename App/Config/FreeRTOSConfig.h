...
#define configUSE_TICKLESS_IDLE                         0                       /* 1: 使能tickless低功耗模式, 默认: 0 (暂时禁用排查启动问题) */
#define configEXPECTED_IDLE_TIME_BEFORE_SLEEP           2                       /* 进入tickless模式前的最小空闲节拍数 */
...
#define configMAX_PRIORITIES                            8                       /* 定义最大优先级数, 最大优先级=configMAX_PRIORITIES-1, 无默认需定义 (减少至8降低内存占用) */
#define configMINIMAL_STACK_SIZE                        100                     /* 定义空闲任务的栈空间大小, 单位: Word, 增至100字 */
#define configMAX_TASK_NAME_LEN                         8                       /* 定义任务名的最大字符数量, 包含'\0'结束符, 默认: 16 (减至8节省内存) */
...
#define configUSE_RECURSIVE_MUTEXES                     0                       /* 1: 使能递归互斥信号量, 默认: 0 (禁用节省内存) */
#define configUSE_COUNTING_SEMAPHORES                   0                       /* 1: 使能计数信号量, 默认: 0 (禁用节省内存) */
#define configUSE_ALTERNATIVE_API                       0                       /* 已弃用!!! */
#define configQUEUE_REGISTRY_SIZE                       0                       /* 定义可以注册的信号量和消息队列的个数, 默认: 0 (禁用节省内存) */
#define configUSE_QUEUE_SETS                            0                       /* 1: 使能队列集, 默认: 0 (禁用节省内存) */
...
/* 内存分配相关定义 */
#define configSUPPORT_STATIC_ALLOCATION                 1                      /* 1: 支持静态申请内存, 默认: 0 (启用静态分配) */
#define configSUPPORT_DYNAMIC_ALLOCATION                1                       /* 1: 支持动态申请内存, 默认: 1 (同时保持动态) */
#define configTOTAL_HEAP_SIZE                           ((size_t)(6144))       /* FreeRTOS堆中可用的RAM总量，增至6KB以支持更多任务和浮点运算 */
...
/* 钩子函数相关定义 */
#define configUSE_IDLE_HOOK                             1                       /* 1: 使能空闲任务钩子函数, 无默认需定义 (用于实现低功耗) */
#define configUSE_TICK_HOOK                             0                       /* 1: 使能系统时钟节拍中断钩子函数, 无默认需定义 */
#define configCHECK_FOR_STACK_OVERFLOW                  2                       /* 1: 使能栈溢出检测方法1, 2: 使能栈溢出检测方法2, 默认: 0 (启用检测防止栈溢出) */
#define configUSE_MALLOC_FAILED_HOOK                    1                       /* 1: 使能动态内存申请失败钩子函数, 默认: 0 (启用检测防止内存不足) */
#define configUSE_DAEMON_TASK_STARTUP_HOOK              0                       /* 1: 使能定时器服务任务首次执行前的钩子函数, 默认: 0 */

/* 运行时间和任务状态统计相关定义 */
#define configGENERATE_RUN_TIME_STATS                   0                       /* 1: 使能任务运行时间统计功能, 默认: 0 (禁用节省内存) */
...
#define configTIMER_TASK_STACK_DEPTH                    80                              /* 定义软件定时器任务的栈空间大小（80字） */
